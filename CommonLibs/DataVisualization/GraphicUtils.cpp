...
void GraphicUtils::DrawColoredSpace(const HostData3D<float>& values, const Vector3D<size_t>& curPlaneNum,
...
void GraphicUtils::DrawDissectedVectorSpace(const HostData3D<Vector3D<float>>& vectors, float minValue, float maxValue,
...
void GraphicUtils::DrawVectorSpace(const HostData3D<Vector3D<float>>& vectors,
																	 float minValue, float maxValue, const Grid3DCoordSys& cs) {
	float normLimit = cs.H();
	float normFactor = normLimit / maxValue;
...
void GraphicUtils::DrawImpurityPlain(const HostData2D<float>& impurities, const HostData2D<bool>& isEarths,
...
void GraphicUtils::GetColorByImpurity(float impurity, bool isEarth, float minImpurity, float maxImpurity, float *rgb) {
	if (isEarth) {
		rgb[0] = 0;
		rgb[1] = 255;
		rgb[2] = 0;
...
void GraphicUtils::DrawParticle(float x, float y, float z, float radius) {
...
void GraphicUtils::DrawCostalCude(const Vector3D<float>& p0, const Vector3D<float>& p1) {
	glBegin(GL_LINES);
