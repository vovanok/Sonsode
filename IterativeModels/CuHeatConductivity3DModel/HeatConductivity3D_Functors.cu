...
class HeatConductivity3DFunctor {
protected:
	float _a, _h, _tau;
	DataKind _data;

public:
	__host__ __device__ HeatConductivity3DFunctor() { }
	HeatConductivity3DFunctor(float a, float h, float tau, DataKind data)
...
GpuDevice& HeatConductivity3DFunctor<DeviceData3D<float>>::gpu() const {
	return _data.gpu();
}