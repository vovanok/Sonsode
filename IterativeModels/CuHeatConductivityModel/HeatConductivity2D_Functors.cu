...
class HeatConductivity2DFunctor {
protected:
	float _a, _h, _tau;
	DataKind _data;

public:
	__host__ __device__ HeatConductivity2DFunctor() { }
	HeatConductivity2DFunctor(float a, float h, float tau, DataKind data)
...
GpuDevice& HeatConductivity2DFunctor<DeviceData2D<float>>::gpu() const {
	return _data.gpu();
}