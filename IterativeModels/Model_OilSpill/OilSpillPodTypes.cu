...
namespace OilSpill {
	...
	struct OilSpillConsts {
		...
		OilSpillConsts(Vector2D<float> ukl, Vector2D<float> beta,
	...
	class OilSpillDataH {
	public:
		friend class OilSpillDataD;

		size_t dimX() { return data.dimX(); }
		size_t dimY() { return data.dimY(); }

		OilSpillDataH() { }
		OilSpillDataH(size_t dimX, size_t dimY)
	...
	public:
		...
		__host__ __device__ OilSpillDataD() { }

		OilSpillDataD(const GpuDevice& gpu, size_t dimX, size_t dimY)
			: data(DeviceData3D<float>(gpu, dimX, dimY, 12)) { }

		OilSpillDataD(const GpuDevice& gpu, OilSpillDataH hostData)
			: data(DeviceData3D<float>(gpu, hostData.data)) { }

		void Erase() { data.Erase(); }
		void TakeFrom(const OilSpillDataH& hostData) { data.TakeFrom(hostData.data); }
		void PutTo(OilSpillDataH& hostData) { data.PutTo(hostData.data); }
